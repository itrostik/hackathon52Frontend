...
export default function Login() {
  ...
  const [title, setTitle] = React.useState('Вход')
  const navigate = useNavigate()

  const handleClick = async () => {
    try {
      const data = await axios.post('http://localhost:4200/api/users.check', {
        email,
        password,
      })
    ...
    <div className="login">
      <div className="login__wrapper">
        ...
        <input
          ...
          className={title === 'Вход' ? 'login__input' : 'login__input error'}
        />
        <input
          type="password"
          name="password"
